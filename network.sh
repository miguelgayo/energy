...
function printHelp() {
  ...
  echo "      - 'deployCC' - deploy the personal chaincode on the channel"
...
function createOrgs() {
  ...
  if [ "$CRYPTO" == "Certificate Authorities" ]; then
    ...
    for i in 0 1 2 3 4 10 11 12 13 14
    do
      createOrg1 $i
    done
    ...
    for i in 1 2 3 4 10 11 12 13 14
    do
      createOrderer $i
    done
...
function networkUp() {
  ...
  docker network create --attachable --driver overlay net_test
...
## call the script to join create the channel and join the peers of org1
...
function networkDown() {
  ...
  docker-compose -f $COMPOSE_FILE_BASE -f $COMPOSE_FILE_CA  -f docker/host2.yaml down --volumes --remove-orphans
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    rm -rf channel-artifacts log.txt personal.tar.gz personal
...
COMPOSE_FILE_BASE=docker/host1.yaml
